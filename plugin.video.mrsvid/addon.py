...
def BadExtension(file_name):
	...
	try:
		...
		if str(extension) == 'jpg' or str(extension) == 'txt' or str(extension) == 'xml' or str(extension) == 'srt' or str(extension) == 'db':  ###### txt for folder comments ! db for thumbs.db which generates automatically by windows!
			return True  
...
def SetFileContextMenu(listitem,path_link,file_name):
	...
	try:
		...
		if FileExtension(file_name)!= 'strm':  ### skip lives!
			contextMenuItems.append(('Download','XBMC.RunScript(special://home/addons/plugin.video.mrsvid/addon.py,ToDownload,' + download_args + ")'"))

...
def BadExtension(file_name):
	...
	try:
		...
		if str(extension) == 'jpg' or str(extension) == 'txt' or str(extension) == 'xml' or str(extension) == 'srt' or str(extension) == 'db' or str(extension) == 'cache' or str(extension) == 'ini' :  ###### txt for folder comments !
			return True  
...
def SetInfos(path_link,file_name):
	...
	dir_list = xbmcvfs.listdir(path_link)	
	if file_name+'.xml' in dir_list[1] : # This check is because of this : when file_name.xml (info file) exists , the internal thumb of the file is not shown!
		try:
			...
			if comment == [] :
				comment = ['']
			duration = common.parseDOM(x, "duration")
			if duration == [] :
				duration = ['']
				
			#xbmcgui.Dialog().ok('Info Label',title[0],genre[0])	
			infos = {'Title' : title[0] , 'Genre': genre[0] , 'Year': year[0] , 'comment':comment[0] , 'artist': artist[0], 'album': album[0],'tracknumber' : track[0] ,'duration' : duration[0] }
...
def SetFileContextMenu(listitem,path_link,file_name):
	...
	try:
		...
		if FileExtension(file_name)!= 'strm':  ### skip lives!
			...
			contextMenuItems.append(('Download [size:%s]' %filesize,'XBMC.RunScript(special://home/addons/plugin.audio.mrsaud/addon.py,ToDownload,' + download_args + ")'"))
...
def CreateFilesFolders(link,base):
	...
	try:
		xbmcgui.Window(10501).setProperty( 'path' ,link) #for insert folder label on the top of music windows (mymusicsongs.xml)
		xbmcplugin.endOfDirectory(int(sys.argv[1]))	
		xbmcplugin.setContent(int(sys.argv[1]), 'songs')
